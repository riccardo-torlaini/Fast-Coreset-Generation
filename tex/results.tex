...
For our real-world datasets, we utilize the following datasets: MNIST \david{which version did you use?}, Song~\cite{song} ($515, 345$ points in dimension 90), and some dataset from the UCI librairie \cite{Dua:2019}: KDD-cup~\cite{kddcup} ($4,000,000$ points in dimension 48) \david{is it this one? Not the same as rejection sampling paper}, Census~\cite{census} ($2, 458, 285$ points in dimension 68), and Cover Type~\cite{covtype} ($581,012$ points in dimension $54$). 
In all real and
...
%so that the HST may separate the points into disjoint cells.
