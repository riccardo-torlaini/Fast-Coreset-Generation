...
an Intel Core i9 10940X 3.3GHz 14-Core processor.
...
This pattern changes in the benchmark dataset as it is devised to challenge sensitivity sampling's reliance on the initial solution. Nonetheless, we see that
sensitivity sampling does not require significantly larger sample sizes before it provides good compression on the benchmark. We lastly show how these
compressions facilitate fast clustering on large datasets in Table~\ref{tbl:lloyds}. We see that running Lloyd's algorithm on the samples obtains a consistent
cost value on the full datasets.
...
composition on runtime in \cref{fig:streaming_runtimes} in the Appendix.}. Although inconsistent with the prevailing intuition, we must therefore conclude that
