...
and so on \david{Are they precisely at $(c, 0, 0, \cdots)$ or very close? if they are precisely there, scaling $c$ should not change. Maybe say $c=1$ and a tiny noise?}. To see that this creates arbitrarily bad lightweight coresets as $c \rightarrow \infty$, consider that the likelihood of hitting each cluster goes to
...
For our real-world datasets, we utilize the following datasets: MNIST \david{which version did you use?}, Song~\cite{song} ($515, 345$ points in dimension 90),
and some dataset from the UCI librairie \cite{Dua:2019}: KDD-cup~\cite{kddcup} ($4,000,000$ points in dimension 48) \david{is it this one? Not the same as
rejection sampling paper}, Census~\cite{census} ($2, 458, 285$ points in dimension 68), and Cover Type~\cite{covtype} ($581,012$ points in dimension $54$).  In
...
% when reporting the sensitivity sampling results, we will be
