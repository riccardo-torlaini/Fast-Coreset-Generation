...
Both start by computing a solution $\calA$. When $\calA$ is a $c$-approximation, both algorithms compute a $c \eps$-coreset of size $\tilde O\lpar
...
guarantees as with a slow $O(1)$-approximation is thus not only an artifact, but a crucial piece of our coreset's success. 
...
distance to previously opened centers. The two key properties of this algorithm are the following: it runs in $\tilde O\lpar n d \log \Delta\rpar$ time, and
...
$k$-means ($z=2$).  We use it, in combination with sensitivity sampling, as follows.
