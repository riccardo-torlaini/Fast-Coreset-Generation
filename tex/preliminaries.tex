...
sample, namely, the number of points in $C_i \cap S$ weighted with $w_p$ \andrew{I'm not sure what `via the sample' means}. A sampled point $p$ in $C_i$ is weighted $\tilde w(p) = \tilde w_1(p) \lpar (1+\eps)|C_i|
...
representative of the dataset as a whole. Although there is a wealth of methods for sampling and coresets in the streaming setting \andrew{references}, we
...
\andrew{citations?} wherein one discretizes the input into blocks and forms a coreset\footnote{These can be coresets or can simply be random samples.} from each
...
level in the tree and then (2) concatenating these coresets and obtaining one final coreset from the concatenation. \andrew{Include error/variance bounds here}
