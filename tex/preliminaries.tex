...
\end{equation}
where $\calC_p$ is the cluster that $p$ belongs to. This is always an upper-bound on the sensitivity values \andrew{ref?}.
...
sample, namely, the number of points in $C_i \cap S$ weighted with $w_p$ \andrew{I'm not sure what `via the sample' means}. A sampled point $p$ in $C_i$ is
...
In terms of other linear-time methods, we are only aware of the lightweight coresets approach\cite{BachemL018}\andrew{True?}, wherein one performs sensitivity
...
equal to the length of the diagonal of the parent hypercube then guarantees that the distance in the tree is within a factor $O(\sqrt{d} \log \Delta)$ of the
distance in the original space, where $\log \Delta$ is the maximum depth of the tree. Lastly, this tree can be constructed in $\tilde{O}(nd \log \Delta)$ time.
