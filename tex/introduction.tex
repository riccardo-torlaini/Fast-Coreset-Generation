...
Uniform sampling has certain provable guarantees for clustering with $k$ centers. For example, for $k=1$, it can result in very accurate results \cite{CLMPS16,CSS21b,InabaKI94} and can be used to compute good clustering under certain conditions \cite{Ben-David07,czumaj2007sublinear,meyerson2004k}.
Unfortunately uniform sampling lacks any sort of worst guarantees for arbitrary values of $k$. Indeed, even for $2$-means, it is easy to construct data sets where uniform sampling must sample the entire data set to obtain a constant factor approximation.
...
To address Question \ref{quest2}, we conduct an extensive experimental evaluation.  We implement generic sensitivity sampling as well as our near linear time
implementation, uniform sampling, and hybrid sampling distributions interpolating between uniform sampling and sensitivity sampling. Popular examples include
\emph{lightweight coresets} \cite{bachem2018scalable}, as well as seeding sensitivity sampling by computing a solution with $k'<k$ centers.
