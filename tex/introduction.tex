...
Therefore, a compression step, also called \textit{sketching}, is often used in preprocessing. Common techniques in this line of work are (random) projections on low dimensional subspaces or sampling algorithms and it is common for the two approaches can of course be combined \cite{MahoneyDMW12} \chris{More citations?}. 
...
Unfortunately uniform sampling has strong guarantees only for very constrained problem formulations. Indeed, even for $2$-means, it is easy to construct data sets where uniform sampling must sample the entire data set to obtain a constant factor approximation.

The alternative are non-unform sampling methods. 
...
\chris{This section reads more like preliminaries to me. Move it there and add some more notation?}
...
In this case, the \textit{group sampling} algorithm developed in \cite{stoc21, stoc22} yields a coreset of size $\tilde{O}( k \eps^{-z-2})$ \footnote{For the special case of $k$-median and $k$-means, the algorithm can further improved to $\tilde{O}(k^{4/3} \eps^{-2})$ and $\tilde{O}( k^{3/2} \eps^{-2})$, respectively, see \cite{}.}, while we know that any coreset must have size $\Omega \lpar k\eps^{-2}\rpar$ \cite{stoc22}.
